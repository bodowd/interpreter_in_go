...
type Parser struct {
	...
	// we will use these maps to check if the appropriate map (infix or prefix) has a parsing function
...
func New(l *lexer.Lexer) *Parser {
	...
	// add these parsing functions to the map
...
// these two functions help with adding entries to the maps
...
const (
	// iota gives the following constants incrementing numbers as values, from 1 to 7 here
	// the order and relation matter; we want certain operators to have higher precedence than others
	_ int = iota
	LOWEST
	EQUALS      // ==
	LESSGREATER // > or <
...
func (p *Parser) parseIdentifier() ast.Expression {
	// sets the value to the curToken Literal
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	// convert into int64 and save to Value field and return the newly
	// constructed *ast.IntegerLiteral node
